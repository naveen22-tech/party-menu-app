// Static menu data for the Party Menu Application.
// No API/API key is used for menu data — everything here is local.
...
/**
 * Filter menu items locally based on the given parameters.
 * @param {Object} params
 * @param {string} [params.category] - starter | main | sides | desert | all
 * @param {string} [params.diet] - veg | nonveg | all
 * @param {string} [params.name] - case-insensitive partial match on dish name
 */
...
/**
 * Look up a single menu item by id (id may be string or number).
 */
export function getMenuItemById(id) {
  return menuData.find((item) => String(item.id) === String(id));
}
